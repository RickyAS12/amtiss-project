...
df_good_consume = db_good_consume.groupby(
    ['asset_category', 'asset_code', 'asset_name','product_id', 'product_name', 'date'], as_index=False
).agg({
...
df_good_consume_2 = df_good_consume.groupby(['asset_category', 'asset_code', 'asset_name','product_name', 'purchase_count'], as_index=False).agg({
    'date': 'max',
    'average_age': 'mean'
...
db_hm_record = db_hm_record.rename(columns={'date': 'latest_asset_used_at'}).groupby(['asset_category', 'asset_code','asset_name'], as_index=False)['latest_asset_used_at'].max()
