...
df = df.groupby(
    ['asset_category', 'asset_code', 'asset_name', 'product_id', 'product_name', 'product_subcategory', 'date', 'consume_id_good_consume'], as_index=False, dropna=False
).agg({
...
filtered_df = filtered_df.sort_values(by=['asset_category', 'asset_code', 'asset_name', 'product_subcategory', 'product_id', 'create_date'])

# Calculate the difference in 'hour_meter'
filtered_df['serviced_when'] = filtered_df.groupby(['asset_category', 'asset_code', 'asset_name', 'product_subcategory', 'product_id'])['hour_meter'].diff()

# Calculate the minimum 'hour_meter' for each group
min_hour_meter = merged_df.groupby(['asset_category', 'asset_code', 'asset_name', 'product_subcategory', 'product_id'])['hour_meter'].transform('min')
...
filtered_df['service_count'] = filtered_df.groupby(
    ['asset_category', 'asset_code', 'asset_name', 'product_subcategory', 'product_id']
...
df_new = filtered_df.groupby(
    ['asset_category', 'asset_code', 'asset_name', 'product_subcategory', 'product_name', 'service_count', 'consume_id_good_consume'], as_index=False, dropna=False
).agg({
...
df_final = df_new[['asset_category', 'asset_code', 'asset_name', 'product_subcategory', 'product_name', 'status', 'service_count', 'latest_product_maintained_at', 'latest_asset_used_at', 'avg_service', 'hours_after_maintained']]
...
if selected_asset_category and selected_asset_code and selected_product_subcategory:
    ...
    filtered_df_st = df_final[df_final['product_subcategory'].isin(selected_product_subcategory)]
else:
