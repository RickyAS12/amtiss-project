...
db_search=db.query(
    'SELECT * FROM amtiss.union_hm_gc ORDER BY date'
    , ttl=600
)
...
db_prod=db.query(
    'SELECT * FROM amtiss.age_product_gc_1'
    , ttl=600
)
...
query_df_value_metrics_hour_meter = db.query(
    f"SELECT * FROM amtiss.union_hm_gc WHERE source = 'hm_record'"
    , ttl=600
)

# 4. Query for values metrics good consume
query_df_value_metrics_good_consume = db.query(
    f"SELECT * FROM amtiss.union_hm_gc WHERE source = 'good_consume'"
    , ttl=600
)
...
with cols[6]:
    option_radio = st.radio('**Grouped :**', ['by Assets', 'by Categories'], index=0, horizontal=True, help="How the chart will be viewed")
...
with cols[2]:
    st.metric('**Total Categories**', value = len(categories), help='Total asset categories in the database')

    df_count_categories_hour_meter = len(query_df_value_metrics_hour_meter['asset_category'].unique())
    ...
    df_count_categories_good_consume = len(query_df_value_metrics_good_consume['asset_category'].unique())
...
with st.expander('**Click to see data distribution for each category**', expanded=False):
    ...
    with cols_exp[0]:
        with st.container(border=True, height=450):
            st.subheader('**Average Total Price Distribution per Category**')
            st.write('')
            st.write('')
    ...
    with cols_exp[1]:
        with st.container(border=True, height=450):
            st.subheader('**Average Work Hour Distribution per Category**')
            st.write('')
            st.write('')
            ...
            bar_chart_hour_meter = alt.Chart(df_hour_meter).mark_bar().encode(
                x=alt.X('mean_hour_meter_per_date:Q'),
